...
const schema = z
  .object({
    ...
    phone: z.string().min(8, "Téléphone invalide (8 chiffres minimum)"),
    licenseNumber: z
      .string()
      .regex(/^[0-9]{12}$/, "Le numéro du permis doit contenir 12 chiffres"),
    licenseType: z.enum(["A", "B", "C", "D", "E"]),
...
export default function DriverForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // 🟦 Validation Zod + Toastify
    const result = schema.safeParse({ ...personalInfo, ...licenseInfo });
    ...
    try {
      ...
      const resultSignup = await signUpAsDriver(formData);
      ...
      if (resultSignup.status === "success") {
        ...
        toast.error("❌ " + resultSignup.status);
      }
    } catch (err: unknown) {
